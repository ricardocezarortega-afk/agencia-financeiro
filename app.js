 ...
 async function renderDashboard() {
  ...
  document.getElementById('content').innerHTML = `
    <div class="cards-grid">
      <div class="card"><div class="card-label"><i class="ti ti-wallet" style="color:#3266ad"></i> Saldo em caixa</div><div class="card-value">${fmt(saldo)}</div><div class="card-sub neutral">Recebido − Despesas</div></div>
      <div class="card"><div class="card-label"><i class="ti ti-trending-up" style="color:#0F6E56"></i> Receita do mês (MRR)</div><div class="card-value" style="color:#0F6E56">${fmt(totalMRR)}</div><div class="card-sub neutral">${totalClientes} clientes ativos</div></div>
      ...
      <div class="card"><div class="card-label"><i class="ti ti-coins" style="color:#3266ad"></i> Lucro líquido</div><div class="card-value">${fmt(lucro)}</div><div class="card-sub neutral">Margem ${totalMRR ? Math.round(lucro/totalMRR*100) : 0}%</div></div>
      ...
      <div class="card"><div class="card-label"><i class="ti ti-users" style="color:#534AB7"></i> Ticket médio</div><div class="card-value">${fmt(ticketMedio)}</div><div class="card-sub neutral">${totalClientes} clientes ativos</div></div>
    </div>
    ...
    <div class="chart-card" style="margin-bottom:20px">
      <div class="chart-title">Fluxo de caixa projetado</div>
  ...
  chartInstances['dashboard'].push(new Chart(document.getElementById('chartRecDesp'), {
    ...
    data:{labels:['Jan','Fev','Mar','Abr','Mai','Jun'],
      datasets:[
        {label:'Receitas',data:[0,0,0,0,0,totalMRR],backgroundColor:'#3266ad'},
        {label:'Despesas',data:[0,0,0,0,0,totalDespesas],backgroundColor:'#e74c3c'}
      ]},
  ...
  chartInstances['dashboard'].push(new Chart(document.getElementById('chartFluxo'), {
    ...
    data:{labels:['Sem 1','Sem 2','Sem 3','Sem 4'],
      datasets:[{label:'Saldo',data:[0,0,0,saldo],borderColor:'#1D9E75',backgroundColor:'rgba(29,158,117,0.08)',fill:true,tension:0.4,pointRadius:5}]},
    options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{ticks:{callback:v=>'R$'+(v/1000).toFixed(0)+'k'}}}}
  }));
}
